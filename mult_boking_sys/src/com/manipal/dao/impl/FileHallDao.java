...
public class FileHallDao implements HallDao {
	...
	public void updateSeatType(SeatType seatType) throws DaoException {
		try {
            ...
            String password = "test";
            con = DriverManager.getConnection(url, username, password);
			String query = "update seattype set seattypedesc = ?,seatfare = ? where seattypeId = ?";
			pstmt = con.prepareStatement(query);
			pstmt.setString(1, seatType.getDescription());
			pstmt.setDouble(2, seatType.getFare());
			pstmt.setString(3, seatType.getSeatTypeId());
			pstmt.executeUpdate();
