...
public class FileUserDao implements UserDao {
	...
	public void addUser(User user) throws DaoException {
		...
		try {
			...
			if (count == 0 || count1 == 0) {
				String query = "insert into users(userid,username,usertype,mobileno,emailid)"
						+ " values(?,?,?,?,?)";
	...
	public void deleteUser(int userId) throws DaoException {
		/*
		 * try { Set<User> users = FileUtil.getContent("users.dat"); for (User u
		 * : users) { if (u.getUserId() == userId) { users.remove(u);
		 * FileUtil.putContent("users.dat", users); break; } } } catch
